...
$root = $Path
# Own repository plus explicitly allowlisted sibling-skill URLs referenced in README.
$ownRepoUrlPattern = '^https://github\.com/h8nc4y/(?:agent-handoff-docs|multi-agent-delegation)(?:\.git)?$'
...
$environmentMarkers = [Environment]::GetEnvironmentVariable('AGENT_HANDOFF_DOCS_PRIVATE_MARKERS')
...
if ($null -ne $gitExe) {
    ...
    ) ("agent-handoff-docs-public-git-" + [Guid]::NewGuid().ToString('N'))
